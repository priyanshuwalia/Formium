...
const BlockRenderer = ({ block, onChange, onDelete, onEnter, dragHandleProps, availableBlocks, allBlocks }: BlockRendererProps) => {
  ...
  const removeLogicRule = (index: number) => {
    onChange(block.id, { logic: (block.logic || []).filter((_, i) => i !== index) });
  };

  const getTriggerLabel = (ruleTriggerId: string) => {
    const t = blocks.find((b) => b.id === ruleTriggerId);
    if (!t) return "Previous block";
    return t.label || `Block ${t.order + 1}`;
  };
  ...
  return (
    <div className="group/block relative -mx-4 px-4 py-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors duration-200">
      ...
      {/* Drag Handle */}
      <div
        className="absolute left-0 top-6 opacity-0 group-hover/block:opacity-100 text-gray-300 cursor-grab active:cursor-grabbing transition-opacity"
