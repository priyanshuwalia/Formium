...
const ResponsePage: React.FC<ResponsePageProps> = ({ params }) => {
  ...
  useEffect(() => {
    ...
    const fetchForm = async () => {
      try {
        ...
        const response = await apiFetch<FullForm>(`/forms/${slug}`);
        setForm(response);

        const initialResponses: ResponsesState = {};
        response.blocks.forEach((block: FormBlock) => {
  ...
  const shouldShowBlock = (block: FormBlock, responses: ResponsesState): boolean => {
    if (!block.logic || block.logic.length === 0) return true;
    return block.logic.some((rule) => {
      const answer = String(responses[rule.triggerBlockId] || "");
  ...
  const handleFileChange = async (
    ...
    try {
      const { uploadUrl, key } = await apiFetch<{ uploadUrl: string; key: string }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ formId: form?.id, filename: file.name, contentType: file.type }),
        },
      );
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {(responses[id] as string) ? (
                            <span className="text-indigo-600 font-medium">
                              Selected:{" "}
                              {(responses[id] as string).split("/").pop()}
                            </span>
                          ) : (
