...
const UserHome: React.FC = () => {
    ...
    const handleTemplateClick = (blocks: FormBlock[]) => {
        sessionStorage.setItem("formium_template_blocks", JSON.stringify(blocks));
    ...
    return (
        <div className="flex-1 p-4 lg:p-8 overflow-y-auto w-full">
            ...
            >
                {themeCtx?.theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            ...
            <section className="mb-10 mt-8">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Link href="/create-form" className="text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:underline flex items-center gap-1">
                        View all templates <ArrowRight size={16} />
                    </Link>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <Link href="/create-form" className="flex flex-col items-center justify-center h-32 bg-white dark:bg-gray-900 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-gray-800 transition cursor-pointer group">
