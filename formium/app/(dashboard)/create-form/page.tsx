...
const CreateForm: React.FC = () => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setInputBlocks((prev) => {
      ...
      return moved.map((b, i) => ({ ...b, order: i }));
    });
  };

  useEffect(() => {
    try {
      const stored = sessionStorage.getItem("formium_template_blocks");
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
          setInputBlocks(parsed);
        }
        sessionStorage.removeItem("formium_template_blocks");
      }
    } catch {
      // ignore parse errors
    }
  ...
  const publishFormWithTitle = async (finalTitle: string) => {
    ...
    try {
      ...
      if (cleanedBlocks.length > 0) {
        ...
        await Promise.all(
          cleanedBlocks.map((block) => apiFetch("/form-blocks", {
            method: "POST",
      ...
      if (form.slug) {
        router.push(`/forms/${form.slug}/published`);
      } else {
