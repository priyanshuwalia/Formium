...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      try {
        const data = await apiFetch<Form[] | { forms: Form[] }>("/forms/dashboard");
              ...
              >
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    <Link href={`/forms/${form.slug}`} className="text-xl font-bold text-gray-800 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors line-clamp-1" title={form.title}>
                      {form.title}
                    </Link>
                    {form.isPublished && (
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-400 dark:text-gray-500 mt-auto pt-4 border-t border-gray-50 dark:border-gray-800">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <Link href={`/forms/${form.slug}/responses`} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium text-xs border border-indigo-200 dark:border-indigo-900 px-2 py-1 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition">
                        View Results
                      </Link>
                    </div>
                  </div>
