...
const MODEL = process.env.ANTHROPIC_MODEL || "claude-sonnet-4-5"; // or "claude-3-5-haiku-latest" for cheaper

export type ResponseTextItem = {
  label: string;
  value: string;
};
...
export async function summarizeResponses(
  formTitle: string,
  itemsByLabel: { label: string; items: string[] }[],
): Promise<AISummary | null> {
  if (!isConfigured()) return null;

  const sample = itemsByLabel.map((f) => ({
    field: f.label,
    answers: f.items.slice(0, 8), // cap sample size to keep calls cheap
  }));
...
Keep themes to max 5, notableResponses to max 3.`;
  ...
  try {
    ...
    // Robust JSON extraction: strip fences if present, find first { ... } block
