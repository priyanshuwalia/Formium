...
const normalizeOpenRouterApiKey = (apiKey) => {
    return apiKey
        .trim()
        .replace(/^Bearer\s+/i, "")
        .replace(/^["']|["']$/g, "")
...
const extractJsonObject = (content) => {
    ...
    throw new ConversationalResponseError("LLM_INVALID_JSON", 502, "The selected OpenRouter model did not return valid structured output. Try again or switch OPENROUTER_MODEL to a chat model that follows JSON instructions.");
};
const parseInterviewResult = (content) => {
    let parsed;
    try {
        ...
        throw new ConversationalResponseError("LLM_INVALID_JSON", 502, "The selected OpenRouter model returned a response I could not read. Please try again.");
    }
    const answers = Array.isArray(parsed.answers)
        ? parsed.answers.filter((answer) => (typeof answer === "object"
            && answer !== null
...
export const createConversationalResponse = async (formId, messages, apiKey, log = () => undefined) => {
    ...
    if (!openRouterApiKey) {
        ...
        throw new ConversationalResponseError("OPENROUTER_API_KEY_REQUIRED", 400, "Your OpenRouter API key is required for conversational responses.");
    }
    ...
    if (!completionResponse.ok) {
        ...
        throw new ConversationalResponseError(`OPENROUTER_REQUEST_FAILED_${completionResponse.status}`, completionResponse.status >= 400 && completionResponse.status < 500 ? completionResponse.status : 502, `OpenRouter rejected the request: ${openRouterMessage || completionResponse.statusText}`);
    }
    ...
    if (!content) {
        throw new ConversationalResponseError("LLM_EMPTY_RESPONSE", 502, "OpenRouter returned an empty model response. Please try again.");
    }
