...
const isBlockVisible = (block, items) => {
    const rules = block.logic;
...
export const createResponse = async (formId, items) => {
    ...
    if (!form)
        throw new ResponseError("Form not found", 404);
    const missing = form.blocks.filter((b) => b.required &&
    ...
    if (form.User) {
        ...
        if (limits.responsesPerMonth !== Infinity) {
            ...
            if (count >= limits.responsesPerMonth) {
                throw new ResponseError("This form has reached its monthly response limit", 403);
            }
        }
    }
    ...
    if (form.User?.email) {
        ...
        sendNewResponseNotification(form.User.email, form.title, form.slug, count).catch((err) => console.error("Response notification failed:", err));
    }
    return response;
};
...
export const buildResponsesCsv = async (formId) => {
    const form = await prisma.form.findUnique({
        where: { id: formId },
